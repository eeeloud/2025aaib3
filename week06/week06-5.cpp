// week06-5.cpp 今天的 LeetCode 挑戰題, 有金幣
// 3350. Adjacent Increasing Subarrays Detection II
// 題目跟昨天的 3349 很像。昨天給你 k 今天問你「最大可能的 k」
...
public:
    int maxIncreasingSubarrays(vector<int>& nums) {
        ...
        for(int i=0; i<N-1; i++){
            int now = min( left[i], right[i+1] ); // 現在這格左邊 combo 右邊 combo都符合
            if(now>ans) ans = now; // 有更大的 k, 就用來更新ans
        }
        return ans;
    }
